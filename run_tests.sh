...
function run_tests {
  ...
  if [ $debug -eq 1 ]; then
    if [ "$testropts" = "" ] && [ "$testrargs" = "" ]; then
      ...
      # provided.
      testrargs="discover ./billingstack/tests"
    fi
  ...
  if [ setup.cfg -nt billingstack.egg-info/entry_points.txt ]
  then
  ...
  if [ $coverage -eq 1 ]; then
    ...
    ${wrapper} coverage html --include='billingstack/*' --omit='billingstack/openstack/common/*' -d covhtml -i
  fi
